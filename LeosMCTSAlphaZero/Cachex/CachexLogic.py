...
class Board():
    ...
    def check_win_move(self, color):
        ...
        while not queue.empty():
            ...
            if not has_start and curr_coord[idx] == 0:
                has_start = True
            elif not has_end and curr_coord[idx] == self.n - 1:
                has_end = True
            if has_start and has_end:
                # print("curr mv: ", self.curr_move)
                # display(self._data, self.n)
                return color
    ...
    def execute_move(self, action, color):
        ...
        if action == self.n * self.n:
            self.swap()
            self.curr_move = (self.curr_move[1],self.curr_move[0])
        # Convention place
        else:
